...
## Workspace Scope
- Applies to the entire Longhorn workspace (`.opencode/` and `openspec/` at root).
- This summary is authoritative; detailed procedures live in `AGENTS.d/*.md`. If a subdoc conflicts, follow this summary.
- Plans must include `OpenSpec: <change-id|N/A>`; openspec entries must reference `Plan: <plan-file>`. Keep both ASCII-only and in sync.
