...
if [ "${JENKINS_COMMON_LOADED:-false}" != true ]; then
  ...
  _JENKINS_COMMON_DIR=$(resolve_script_path "${BASH_SOURCE[0]}")
  _JENKINS_WORKSPACE_ROOT=$_JENKINS_COMMON_DIR
  while [ ! -f "$_JENKINS_WORKSPACE_ROOT/AGENTS.md" ] || [ ! -d "$_JENKINS_WORKSPACE_ROOT/agent-skills" ] || [ ! -d "$_JENKINS_WORKSPACE_ROOT/repo" ]; do
    _JENKINS_PARENT_DIR=${_JENKINS_WORKSPACE_ROOT%/*}
    [ -n "$_JENKINS_PARENT_DIR" ] || _JENKINS_PARENT_DIR=/
    if [ "$_JENKINS_PARENT_DIR" = "$_JENKINS_WORKSPACE_ROOT" ]; then
      printf '%s\n' "Longhorn workspace root not found" >&2
      return 3
    fi
    _JENKINS_WORKSPACE_ROOT=$_JENKINS_PARENT_DIR
  done
  cd "$_JENKINS_WORKSPACE_ROOT"
  if ! declare -f defensive_parse_args >/dev/null 2>&1; then
    # shellcheck source=/dev/null
    . "$_JENKINS_WORKSPACE_ROOT/scripts/lib/defensive_prelude.sh"
  fi
  JOBS_FILE="$_JENKINS_WORKSPACE_ROOT/agent-skills/jenkins-ops/jobs.json"
  EXIT_JENKINS=4
  EXIT_TIMEOUT=5
