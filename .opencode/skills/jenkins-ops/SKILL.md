...
Use only `.opencode/skills/jenkins-ops/scripts/` for Jenkins HTTP operations and job-host SSH. The wrappers enforce the allowlist, live parameter metadata, private evidence, bounded diagnosis, and controlled access.
...
Prefix every wrapper with `.opencode/skills/jenkins-ops/scripts/` from the workspace root.
...
```bash
.opencode/skills/jenkins-ops/scripts/trigger_job.sh e2e 'CUSTOM_TEST_OPTIONS=-t "Backup Listing With More Than 1000 Backups" --exclude "cluster" -v DATA_ENGINE:v2'
```
