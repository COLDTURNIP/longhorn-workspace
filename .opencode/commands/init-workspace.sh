...
# Wrapper defaults to execute mode unless explicitly overridden.
...
REPO_INIT_SCRIPT=".opencode/commands/repo-init.sh"
  ...
  "indexers": [
    {
      ...
      "script": ".opencode/skills/interaction-mapper/map_interactions.sh",
...
PY
    ...
    if [[ "$script" != .opencode/skills/* ]]; then
      warn "Indexer $name rejected: script outside allowlist prefix"
      outputs_json=$(build_outputs_json "$outputs_json_declared")
      append_indexer_result "$name" "$script" "$MODE" 3 "failed" "script outside .opencode/skills/" "$outputs_json" ""
      continue
    fi
    if [ ! -f "$script" ]; then
