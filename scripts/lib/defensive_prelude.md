...
```
scripts/lib/defensive_prelude.sh
```
...
while [ ! -f "$WORKSPACE_ROOT/AGENTS.md" ] || [ ! -d "$WORKSPACE_ROOT/agent-skills" ]; do
...
For a full reference implementation, see `scripts/lib/defensive_prelude.sh` and the updated skills (repo-init, ticket-sanitizer, sync-crd-helm, interaction-mapper, support-bundle-analysis/extract-bundle, verify-setup).
